...
# 키워드 추출 에이전트 import 시도
try:
    from agent.keyword_extractor.agent import keyword_extractor_agent
    from agent.keyword_extractor.tools import extract_keywords, upsert_review, search_reviews_by_keyword, KeywordExtractorTools
    AGENT_AVAILABLE = True
except ImportError as e:
    print(f"Keyword Extractor Agent import 실패: {e}")
...
# 키워드 분석 결과 저장용 session state
if 'keyword_analysis_results' not in st.session_state:
    st.session_state.keyword_analysis_results = {}

# 키워드 데이터베이스 초기화
if AGENT_AVAILABLE:
    try:
        db_tools = KeywordExtractorTools("ecommerce_reviews.db")
...
for comment in reversed(st.session_state.comments):
    with st.container():
        ...
        with col4:
            if st.button("🔍 키워드 분석하기", key=f"review_{comment['id']}", type="primary", use_container_width=True):
                if AGENT_AVAILABLE:
                    with st.spinner("키워드 분석 중..."):
                        try:
                            # 에이전트를 통한 키워드 추출 실행 (디버깅 정보 포함)
                            agent_prompt = f"다음 리뷰에서 제품 특성 키워드를 추출해주세요: {comment['content']}"

                            # 에이전트 실행 (올바른 메소드 사용)
                            try:
                                agent_response = keyword_extractor_agent.run(agent_prompt)
                                raw_response = str(agent_response)
                            except AttributeError:
                                # run 메소드가 없으면 직접 툴 호출
                                keyword_result = extract_keywords(comment['content'])
                                extracted_keywords = keyword_result.get('keywords', [])
                                raw_response = f"직접 툴 호출 결과: {keyword_result}"
                                agent_prompt = f"직접 툴 호출: extract_keywords('{comment['content']}')"
                            else:
                                # 툴 결과에서 키워드 추출
                                extracted_keywords = []
                                ...
                                try:
                                    # JSON 형태로 파싱 시도
                                    if 'keywords' in raw_response:
                                        import re
                                        json_match = re.search(r'\{[^}]*"keywords"[^}]*\}', raw_response)
                                        if json_match:
                            ...
                            if extracted_keywords:
                                upsert_review(comment['id'], comment['content'], extracted_keywords)

                            # 키워드 분석 결과를 comment별로 저장 (디버깅 정보 포함)
                            st.session_state.keyword_analysis_results[comment['id']] = {
                    ...
                    st.warning("키워드 분석 에이전트를 사용할 수 없습니다.")

        # 키워드 분석 결과가 있으면 펼치기/접기 메뉴 표시
        if comment['id'] in st.session_state.keyword_analysis_results:
            keyword_result = st.session_state.keyword_analysis_results[comment['id']]
            keywords = keyword_result.get('keywords', [])

            status_icon = "🏷️" if keywords else "📝"
            result_text = f"키워드 {len(keywords)}개 추출" if keywords else "키워드 없음"

            with st.expander(f"{status_icon} 키워드 분석 결과 - {result_text}", expanded=False):
                # 분석 시간
                st.write(f"**분석 시간:** {keyword_result['timestamp']}")
                ...
                if keywords:
                    ...
                    for keyword in keywords:
                        try:
                            related_reviews = search_reviews_by_keyword(keyword)
                            if related_reviews:
                                with st.expander(f"'{keyword}' 관련 리뷰 {len(related_reviews)}개", expanded=False):
                                    for review in related_reviews[:3]:  # 최대 3개만 표시
                                        st.write(f"- {review['text'][:100]}...")
                        except Exception as e:
                            st.error(f"'{keyword}' 검색 중 오류: {str(e)}")
                else:
                    st.info("이 리뷰에서는 제품 특성 관련 키워드가 발견되지 않았습니다.")

                # 원본 리뷰 텍스트
                with st.expander("원본 리뷰 텍스트", expanded=False):
                    st.code(keyword_result.get('review_text', '텍스트 없음'), language='text')

                # 에이전트 디버깅 정보
                if 'agent_raw_response' in keyword_result:
                    with st.expander("에이전트 디버깅 정보", expanded=False):
                        st.write("**에이전트에게 보낸 프롬프트:**")
                        st.code(keyword_result.get('agent_prompt', '프롬프트 없음'), language='text')

                        st.write("**에이전트 원본 응답:**")
                        st.code(keyword_result.get('agent_raw_response', '응답 없음'), language='text')
...
with st.form("comment_form"):
    ...
    if submitted:
        if author_name and comment_content:
            ...
            # 자동 키워드 분석 수행 (AGENT_AVAILABLE인 경우)
            if AGENT_AVAILABLE:
                with st.spinner("새 댓글 자동 키워드 분석 중..."):
                    try:
                        # 키워드 추출 실행
                        keyword_result = extract_keywords(comment_content)
                        extracted_keywords = keyword_result.get('keywords', [])

                        # 데이터베이스에 저장
                        if extracted_keywords:
                            upsert_review(new_comment['id'], comment_content, extracted_keywords)

                        # 키워드 분석 결과를 comment별로 저장
                        st.session_state.keyword_analysis_results[new_comment['id']] = {
                            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "keywords": extracted_keywords,
                        ...
                        st.error(f"자동 키워드 분석 중 오류가 발생했습니다: {str(e)}")
