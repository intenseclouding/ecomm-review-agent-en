...
PROFANITY_PROMPT = """
    ...
    Moderation criteria:
    1. Profanity, slang, offensive language
    2. Sexual or suggestive expressions
    3. Hate speech, discriminatory expressions
    4. Threatening or violent expressions
    5. Spam or promotional content

    Please make judgments considering the subtle nuances and context of the language.

    Please provide your response only in the following JSON format. Do not include backticks or code block formatting (```json, ```python, etc.) in your response:
    {
...
IMAGE_MATCH_PROMPT = """
    ...
    1. Does the image show the product or a related product?
...
RATING_CONSISTENCY_PROMPT = """
    Please analyze whether the review rating and content are consistent:
    ...
    4. Understand context and nuances of the language

    Judgment criteria:
    - Rating 4-5 stars: Positive content expected
    - Rating 1-2 stars: Negative content expected
    - Rating 3 stars: Neutral content expected
...
def check_image_product_match(image_path: str, product_data: Dict) -> Any:
    ...
    try:
        if not image_path or not os.path.exists(image_path):
            return {
                "status": "SKIP",
                "reason": "No uploaded image available.",
                "confidence": 1.0,
            }
