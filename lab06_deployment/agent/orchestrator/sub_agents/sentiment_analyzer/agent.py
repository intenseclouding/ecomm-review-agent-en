...
SYSTEM_PROMPT = """
    ...
    Please analyze the review text to classify the sentiment and assign a score.
    ...
    <guidelines>
    - Carefully detect sarcasm and ironic expressions ('really great' + negative context)
    - When mixed emotions are present, identify the overall dominant sentiment
    - Consider Korean euphemistic expressions: 'not bad' is positive, 'so-so' is moderate satisfaction
    - Understand the context of domain-specific terminology (e.g., 'difficulty' in games can be positive)
    - Consider the relative meaning of comparative expressions ('better than ~')
    - Set confidence low when the text is too short or ambiguous
    </guidelines>

    <output_format>
    Please return the result in JSON format. Do not add backticks or code block formatting (```json, ```python, etc.) to the response. :
    {
    "sentiment": "positive|negative|neutral",
    "score": 0.8,
    "confidence": 0.9,
    "reason": "Basis for analysis"
    }
    </output_format>
"""
# Sentiment Analysis Agent creation
